...
GeoExt.ux.GeoNamesSearchCombo = Ext.extend(Ext.form.ComboBox, {
     ...
     *  default value is "Search location in Geonames".
     */
    locationIcon: 'http://ilwac.ige.fr/geonetwork/apps/images/viseur.png',
     ...
     */
    tpl: '<tpl for="."><div class="x-combo-list-item" lon={lng} lat={lat}><h1>{name}<br></h1>{fcodeName} - {countryName}</div></tpl>',
    ...
    initComponent: function() {
        ...
        this.mplace1 = new OpenLayers.Layer.Markers('findplace', {visibility:false,displayInLayerSwitcher: false});

		this.map.addLayer(this.mplace1);
		this.mplace1.setZIndex(this.map.Z_INDEX_BASE['Popup'] - 1);
		
		var msize = new OpenLayers.Size(48,48);
		var moffset = new OpenLayers.Pixel(-(msize.w/2), - (msize.h/2));
		var micon1 = new OpenLayers.Icon(this.locationIcon,msize,moffset);
        ...
        if(this.zoom > 0) {
            this.on("select", function(combo, record, index) {
                ...
                this.mplace1.setVisibility(false);
            }, this);
            this.on("expand", function(combo) {
            	var me=this;
            	combo.innerList.on('mouseover', function(e, t) {
				    ...
				    if (item!=null)
				    {
				      var m_lat = item.getAttribute('lat');
				      var m_lon = item.getAttribute('lon');
				      var m_lonlat = new OpenLayers.LonLat(m_lon,m_lat);
				      m_lonlat.transform(
	                    new OpenLayers.Projection("EPSG:4326"),
	                    me.map.getProjectionObject()
				      );
				      
				      me.mplace1.clearMarkers();
				      marker1 = new OpenLayers.Marker( m_lonlat,micon1);
				      me.mplace1.addMarker(marker1); 
				      me.mplace1.setVisibility(true);
				      this.mplace1.setZIndex(this.map.Z_INDEX_BASE['Popup'] - 1);
				    }
				  }, combo);
            }, this);
