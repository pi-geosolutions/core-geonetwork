...
<xsl:stylesheet version="2.0"
  ...
  <!--
     ...
     * jsonp           -  Enable JSONP; the JSON output will be prepended with
     ...
     Source ref & doc:
       see https://github.com/bramstein/xsltjson
      
     Modified by Jean Pommier
  ...
  <xsl:param name="data-path" as="node()" select="/root/response/."/>
  ...
  <xsl:template match="/*">
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:value-of select="json:generate($data-path)"/>
        </xsl:otherwise>
      </xsl:choose>
  </xsl:template>
